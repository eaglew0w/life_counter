---
description: mainリリース（feature/developからmainへのマージとリリース成果物の作成）の手順
trigger: always_on
---
...
## 2. 開発環境への合流 (develop)
1.  **PR作成**: `feature/issue-[Issue番号]` から `feature/develop` へのプルリクエストを作成すること。
2.  **マージ**: PRが承認されたらマージしてブランチを削除すること。

## 3. 本番環境へのリリース (main)
1.  **リリースPR作成**: `feature/develop` から `main` へのプルリクエストを作成すること。
...
## 7. 後処理
1.  ローカルの `main` および `feature/develop` を最新の状態に更新（fetch/pull）すること。
