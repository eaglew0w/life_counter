...
- **ライフ増減**:
  - `+` / `-` ボタンをタップしてライフを増減します。
  - 連続してタップすると、変更量の合計（例: `+5`）が一時的に表示されます。
- **リセット**:
  - 画面中央のリセットボタンを**長押し**すると、以下の項目がすべて初期化されます。
    - ライフ (20)
    - 毒 (Poison) / 速度 (Speed) カウンター
    - 変更量表示
- **テーマ切り替え**:
  - 画面左上の太陽/月アイコンをタップすると、ライトモード/ダークモードが切り替わります（アニメーション付き）。

### 追加カウンター

- **毒 (Poison)**: 毒カウンターの増減・リセットが可能です。
- **速度 (Speed)**: 0〜4 の範囲で速度やエネルギーを管理できます。
